...
export default function AdminDashboard() {
  ...
  const handleLogout = async () => {
    try {
      if (confirm("Logout dari sistem Beefirst?")) {
        ...
        setSession(null)
        // Balik ke /admin agar tidak 404, karena form login ada di rute ini
  ...
  if (!session) return (
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6 text-black font-sans">
      <div className="w-full max-w-md bg-white p-10 rounded-[3rem] shadow-2xl border-4 border-slate-800">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em] mt-2 text-black">Beefirst Visual Access</p>
        </div>
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <button type="submit" className="w-full bg-slate-900 text-white py-5 rounded-2xl font-black uppercase text-xs tracking-[0.3em] shadow-2xl hover:brightness-125 active:scale-95 transition-all text-white">
            Enter Dashboard
          </button>
        </form>
      </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col lg:flex-row text-slate-900 font-sans">
      ...
      <div className={`fixed inset-0 z-[110] lg:relative lg:z-10 transition-transform duration-300 ${isMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0 lg:flex'}`}>
        ...
        <aside className="relative w-72 bg-white border-r-2 border-slate-200 h-full p-8 flex flex-col shadow-2xl lg:shadow-none overflow-y-auto text-black">
          <div className="mb-10 hidden lg:block">
            {settings.logo_url && <img src={settings.logo_url} className="h-12 object-contain mb-4" />}
            <h2 className="font-black text-2xl tracking-tighter uppercase italic text-black">Beefirst</h2>
          </div>
      ...
      {/* KONTEN */}
      <main className="flex-1 p-6 lg:p-12 pt-28 lg:pt-12 overflow-x-hidden text-black font-sans">
        <div className="max-w-6xl mx-auto">
          <header className="flex justify-between items-center mb-10">
            <h1 className="text-4xl font-black tracking-tighter capitalize leading-none italic text-black">{activeTab}</h1>
          ...
          {activeTab === 'overview' && (
            <div className="space-y-10 text-black">
              ...
              <div className="bg-white p-6 md:p-10 rounded-[2.5rem] shadow-xl border border-slate-100 text-black">
          ...
          {activeTab === 'list' && (
            <div className="bg-white rounded-[2rem] shadow-xl border border-slate-100 overflow-hidden text-black">
              <div className="overflow-x-auto">
                <table className="w-full text-left min-w-[700px] text-black uppercase font-sans">
                  <thead className="bg-slate-50 border-b-2 border-slate-200 font-black uppercase text-[10px] tracking-widest text-slate-900 text-center">
                    <tr><th className="p-6 text-black">Customer</th><th className="p-6 text-black">Layanan</th><th className="p-6 text-black">Waktu</th><th className="p-6 text-black">Aksi</th></tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 font-bold text-slate-900 uppercase">
                    {bookings.map(b => (
                      <tr key={b.id} className="text-xs text-center">
                        <td className="p-6 font-black uppercase text-xs text-black">{b.customer_name}<p className="text-[10px] text-slate-400 font-normal">{b.customer_wa}</p></td>
                        ...
                        <td className="p-6 text-[10px] uppercase font-mono text-black">{b.booking_date} | {b.booking_time}</td>
          ...
          {activeTab === 'services' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 text-black">
              <form onSubmit={handleAddService} className="bg-white p-8 rounded-[2.5rem] border-2 border-slate-200 space-y-5 h-fit shadow-xl">
                <h3 className="font-black text-slate-900 uppercase text-xs tracking-widest mb-4 italic text-black">Layanan Baru</h3>
                <input value={newService.name} onChange={e => setNewService({...newService, name: e.target.value})} className="w-full border-2 border-slate-200 p-4 rounded-2xl font-black bg-slate-50 text-sm text-black uppercase" placeholder="e.g. Foto Katalog" />
                ...
                <button type="submit" className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg text-white">Simpan Jasa</button>
              </form>
              <div className="lg:col-span-2 bg-white rounded-[2.5rem] border border-slate-100 shadow-xl overflow-hidden text-black">
                <table className="w-full text-left font-black text-black">
                  <thead className="bg-slate-100 border-b-2 border-slate-200 font-black text-slate-900 text-[10px] uppercase tracking-widest">
                    <tr><th className="p-6 text-black">Nama Jasa</th><th className="p-6 text-right text-black">Harga</th><th className="p-6 text-center text-black">Hapus</th></tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 font-black text-slate-900 uppercase text-xs text-black">
                    {services.map(s => (
                      <tr key={s.id}>
                        <td className="p-6 text-black">{s.name}</td>
                        <td className="p-6 text-right font-mono text-black">Rp {s.price.toLocaleString()}</td>
          ...
          {activeTab === 'settings' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 text-black">
              <div className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-100 flex flex-col items-center">
                <h3 className="font-black text-[10px] uppercase tracking-widest mb-6 text-black italic">Logo Branding</h3>
                ...
                <label className="cursor-pointer bg-slate-900 text-white px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg active:scale-95 transition-all text-white">
                  {uploading ? 'Proses...' : 'Upload Logo'}
                  <input type="file" className="hidden" accept="image/*" onChange={handleLogoUpload} disabled={uploading} />
                </label>
              </div>
              <div className="lg:col-span-2">
                <form onSubmit={handleUpdateSettings} className="bg-white p-8 md:p-12 rounded-[3.5rem] shadow-xl border border-slate-100 space-y-6 text-black uppercase">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 font-black">
                    <div className="space-y-1 italic">
                      <label className="text-[10px] font-black uppercase tracking-widest ml-1 text-black">Nama Bisnis</label>
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-black uppercase tracking-widest ml-1 italic text-black">WhatsApp Admin</label>
                      <input value={settings.whatsapp_admin} onChange={e => setSettings({...settings, whatsapp_admin: e.target.value})} className="w-full border-2 border-slate-200 p-5 rounded-3xl font-black text-slate-900 bg-white font-mono" />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[10px] font-black uppercase tracking-widest ml-1 italic text-black">Warna Branding</label>
                      <div className="flex gap-4 items-center bg-slate-50 p-3 rounded-2xl border-2 border-slate-200 shadow-inner">
                        <input type="color" value={settings.primary_color} onChange={e => setSettings({...settings, primary_color: e.target.value})} className="h-12 w-20 rounded-xl cursor-pointer border-2 border-black" />
                        <span className="text-slate-900 font-black uppercase font-mono text-xs italic">{settings.primary_color}</span>
                      </div>
                    </div>
                    <div className="space-y-1 italic">
                      <label className="text-[10px] font-black uppercase tracking-widest ml-1 text-black">Besar DP (Rp)</label>
                      <input type="number" value={settings.dp_amount} onChange={e => setSettings({...settings, dp_amount: Number(e.target.value)})} className="w-full border-2 border-slate-200 p-5 rounded-3xl font-black text-slate-900 bg-white" />
                    </div>
                  </div>
                  <button type="submit" className="w-full bg-slate-900 text-white py-6 rounded-3xl font-black uppercase text-xs tracking-[0.4em] shadow-2xl active:scale-95 transition-all text-white border-4 border-slate-900">Update Pengaturan</button>
                </form>
              </div>
            </div>
          )}
...
function SidebarItem({ active, icon, label, onClick }: any) {
  return (
    <button onClick={onClick} className={`w-full flex items-center gap-4 p-4 rounded-xl font-black transition-all uppercase text-[10px] tracking-widest ${active ? 'bg-slate-900 text-white shadow-xl translate-x-1 scale-105 border-2 border-black' : 'text-slate-400 hover:bg-slate-50 hover:text-slate-900'}`}>
      {icon} {label}
    </button>
  )
}
...
function FilterBtn({ label, active, onClick }: any) {
  return (
    <button onClick={onClick} className={`px-4 py-2 rounded-xl text-[9px] font-black transition-all ${active ? 'bg-white text-slate-900 shadow-md text-black border-2 border-black' : 'text-slate-400 hover:text-slate-600'}`}>
      {label}
    </button>
  )
}
...
function StatCard({ label, value, color, bg }: any) {
  return (
    <div className="bg-white p-10 rounded-[2.5rem] shadow-xl border-4 border-black flex flex-col gap-5 hover:scale-105 transition-all text-black">
